...
; GENERATE-COLORS
; returns a list of clauses where there is at most one true clause between
; all pairs of current color and colors
;
(defun generate-colors (current n c k)
  (cond ((> c k) NIL)
        (t (cons (list (- (node2var n current k)) (- (node2var n c k))) (generate-colors current n (+ c 1) k)))))
...
(defun at-most-one-color (n c k)
  (cond ((> c k) NIL)
        ((< k 1) NIL)
        (t (append (generate-colors c n (+ c 1) k) (at-most-one-color n (+ c 1) k)))))
...
; returns constraints to make sure that x and y don't have the same color c 
; as upper-bound color k
;
(defun all-different-colors (x y c k)
  (cond ((> c k) NIL)
        (t (cons (list (- (node2var x c k)) (- (node2var y c k))) (all-different-colors x y (+ c 1) k)))))
...
(defun generate-edge-clauses (e k)
  (all-different-colors (car e) (cadr e) 1 k))
...
;(graph-coloring-to-sat "graph1.txt" "output2.txt" 4)

;(graph-coloring-to-sat "graph2.txt" "output3.txt" 3)
;(graph-coloring-to-sat "graph2.txt" "output4.txt" 4)
;(graph-coloring-to-sat "graph2.txt" "output5.txt" 5)
;(graph-coloring-to-sat "graph2.txt" "output6.txt" 6)
;(graph-coloring-to-sat "graph2.txt" "output7.txt" 7)
;(graph-coloring-to-sat "graph2.txt" "output8.txt" 8)